...
const AddCarForm = () => {
  ...
  const onMultiImagesDrop = (acceptedFiles) => {
    const validFiles = acceptedFiles.filter((file) => {
      if (file.size > 5 * 1024 * 1024) {
        toast.error(
          `${file.name} exceeds the file size limit of 5MB and will be skipped`
        );
        return false;
      }
      return true;
        ...
        <TabsContent value="manual" className={"mt-6"}>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Enter the deatils of the car you want to Add.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div>
                  <Label
                    htmlFor="images"
                    className={imageErrors ? "text-red-500" : ""}
                  >
                    Images{" "}
                    {imageErrors && <span className="text-red-500">*</span>}
                  </Label>

                  <div
                    {...getMultiImageRootProps()}
                    className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer hover:bg-gray-50 transition mt-2 ${
                      imageErrors ? "border-red-500" : "border-gray-300"
                    }`}
                  >
                  ...
                  {imageErrors && (
                    <p className="text-xs text-red-500 mt-1">{imageErrors}</p>
                  )}
                </div>
